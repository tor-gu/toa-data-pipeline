...
The data covers only the largest connected component of the match graph, which is the fist thign this lambda needs to compute.

This dataset is shaped exactly for the viz component to consume. Some peculiarities of this set to note:
- The date axis starts with a synthetic pre-season date, the day before the first real, so that playback can begin before the first mach.
